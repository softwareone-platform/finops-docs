# View and Manage Policies

The Anomaly detection page displays all resource count and expense anomaly policies you have created. You can get detailed information for a policy by selecting the policy name. This includes general information, the date of violation, status, average, and actual counts.&#x20;

From the policy details page, you can also edit the policy name and delete the policy.

<div data-with-frame="true"><figure><img src="../../.gitbook/assets/image (3).png" alt=""><figcaption><p>The details page of an anomaly detection policy.</p></figcaption></figure></div>

## Renaming a policy

To rename a policy:

1. Navigate to the **Anomaly detection** page.
2. Select the policy to rename.
3. On the policy details page, select the edit icon <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#1f1f1f&#x22;><path d=&#x22;M200-200h57l391-391-57-57-391 391v57Zm-80 80v-170l528-527q12-11 26.5-17t30.5-6q16 0 31 6t26 18l55 56q12 11 17.5 26t5.5 30q0 16-5.5 30.5T817-647L290-120H120Zm640-584-56-56 56 56Zm-141 85-28-29 57 57-29-28Z&#x22;/></svg>" data-size="line">.
4. Under **Name**, enter the new name and save your changes.

## Deleting a policy

To delete an existing anomaly detection policy:

1. Navigate to the **Anomaly detection** page.
2. Select the policy to delete.
3. On the policy details page, select **Delete**.
4. In the **Delete anomaly Detection Policy** dialog, select **Delete** to confirm the action.
